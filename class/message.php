...
class ImmessageMessage extends IcmsPersistableObject {
	...
	public function __construct(){
		...
		$this->quickInitVar('message_from_uid', XOBJ_DTYPE_INT, false, _CO_IMMESSAGE_MESSAGE_FROM_UID,_CO_IMMESSAGE_MESSAGE_FROM_UID_DSC, $xoopsUser->getVar('uid'));
    ...
    private function message_status(){
    	...
    	switch($this->getVar('message_status','e')){
    		case _IMMESSAGE_STATUS_SEND:
    			if($xoopsUser->getVar('uid') == $this->getVar('message_from_uid','e')){
...
class ImmessageMessageHandler extends IcmsPersistableObjectHandler {
	...
	public function __construct($db){
		$this->IcmsPersistableObjectHandler($db, 'message', 'message_id', 'message_title', 'message_content', 'immessage');
	}
